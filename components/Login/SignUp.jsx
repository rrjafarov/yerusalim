...
// * OTP succes popup var burda asagida
"use client";
import React, { useState } from "react";
import Link from "next/link";
import { useRouter } from "next/navigation";
...
const SignUp = ({ t }) => {
  ...
  const [showSuccess, setShowSuccess] = useState(false); // ✅ əlavə
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setRegisteredEmail(form.email.value);
        setStep("otp");
      } else if (res.status === 400 && data.errors) {
        const msgs = Object.values(data.errors)
          .flatMap((v) => (Array.isArray(v) ? v : [v]))
  ...
  const handleOtpSubmit = async () => {
    if (!otp.trim()) {
      setOtpError(t?.formValidationOtp || "OTP kodu daxil edin");
      return;
    }
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/verify-email-otp`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (res.ok) {
        setShowSuccess(true); // ✅ popup göstər
        setTimeout(() => {
          setShowSuccess(false);
          router.push("/login"); // ✅ 1.5s sonra login-ə yönləndir
        }, 1500);
      } else {
        setOtpError(data.message || t?.invalidOtp || "OTP kodu yanlışdır");
      }
    } catch {
  ...
  if (step === "otp") {
    return (
      <div className="signup">
        <div className="container">
          <h1>{t?.verifyEmail || "E-poçtu təsdiq edin"}</h1>
          <p>{t?.verifyEmailSubTitle || "E-poçtunuza göndərilən OTP kodu daxil edin"}</p>
          ...
          <div className="signupForm">
            <form onSubmit={(e) => e.preventDefault()}>
              ...
              <div className="signupFormGroup">
                {otpError && (
                  <div className="validationMessage">{otpError}</div>
                )}
                <div className={`floatingInput ${otpError ? "inputError" : ""}`}>
                  <input
                    type="text"
                    name="otp_code"
                    placeholder=" "
                    ...
                    maxLength={6}
                  />
                  <label>{t?.otpCode || "OTP Kodu"}</label>
                </div>
              </div>
              ...
              <button
                ...
                style={{
                  backgroundColor: otpLoading ? "transparent" : undefined,
                  border: otpLoading ? "1px solid #e0d5e2" : undefined,
                  pointerEvents: otpLoading ? "none" : undefined,
                }}
              >
                {otpLoading ? (
                  <div
        ...
        {/* ✅ Success Popup */}
        {showSuccess && <SuccessPopup message={t?.signUpSuccessTitle} />}
      </div>
    );
  }
        ...
        <div className="signupForm">
          <form onSubmit={handleSubmit}>
            ...
            <div className="signupFormGroup">
              ...
              <div
                className={`floatingInput ${errors.phone ? "inputError" : ""} ${
                  phone || phoneTouched ? "floating" : ""
                }`}
              >
                <input
