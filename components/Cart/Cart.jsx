...
// ! bu chatgpt-nin kodudur Nurlannan biz bunu deyisdik MEET
...
//   const finalTotal =
//     Number(cartAmount) + (deliveryInfo.isFree ? 0 : Number(deliveryInfo.price));

//   const getPaymentMethodBody = () => {
//     if (selectedPaymentMethod === "card") return "full_payment";
//     return "cash_payment";
//   };
...
//       return {
//         product_order_id: item?.product_order_id ?? item?.id ?? "",
//         product_id: product?.id ?? "",
//         product_name: product?.name ?? "",
//         product_price: item?.price ?? 0,
//         product_old_price: product?.product_variants?.[0]?.old_price ?? 0,
//         quantity: String(item?.qty ?? 1),
//         product_code: product?.code ?? product?.product_code ?? "",
//         product_image: {
//           image,
//         },
//         product_brand: product?.brand?.name ?? product?.brand ?? "",
//         product_brand_logo:
//           product?.brand?.logo ?? product?.brand_logo ?? "",
//       };
//     });
//   };

//   const getGuestUserPayload = () => {
//     if (isUser) {
//       return {
//         full_name: selectedAddress?.name ?? "",
//         phone: selectedAddress?.tel ?? "",
...
//     if (isUser && !selectedAddress) return;

//     if (
//       !isUser &&
//       (!guestFormData.fullName ||
//         !guestFormData.phone ||
//         !guestFormData.region_id ||
...
//         key: "orders",
//         title: isUser
//           ? selectedAddress?.name || userData?.name || ""
//           : guestFormData.fullName,
//         form_data: JSON.stringify({
//           order_products: buildOrderProducts(),
//           amount: finalTotal,
//           guest_user: getGuestUserPayload(),
//           payment_method: selectedPaymentMethod === "card" ? "Online" : "Cash",
//         }),
//       };
...
//       if (!orderId) {
//         throw new Error("order_id alınmadı");
//       }

//       await axiosInstance.post(`make-payment/${orderId}`, {
...
//                     {isSubmitting ? t?.sendingLoad : t?.completeOrder}
//                   </button>
//                 </div>
//               </div>
//             </div>
...
const Cart = ({ t, delveryRegions }) => {
  ...
  const buildOrderProducts = () => {
    return cartProducts.map((item) => {
      ...
      return {
        ...
        quantity: String(item?.qty ?? 1),
        product_code:
          product?.product_variants?.[0]?.product_code ??
          product?.code ??
        ...
        product_brand: product?.brand?.name ?? product?.brand ?? "",
        product_brand_logo: product?.brand?.logo ?? product?.brand_logo ?? "",
      };
    });
  };
  ...
  const handleCompleteOrder = async () => {
    ...
    try {
      ...
      const paymentMethodLabel =
        selectedPaymentMethod === "card" ? "Online" : "cash";
  ...
  return (
    <div className="cartPage">
      {showSuccessPopup && <SuccessPopup message={t?.successCashPayment} />}
